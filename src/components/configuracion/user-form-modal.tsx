...
const userFormSchema = z.object({
  ...
  password: z.string().optional(), // Optional for editing non-admin users if no change
  confirmPassword: z.string().optional(),
}).refine(data => {
  // Password is required for new users (when editingUser is null)
  // For existing users, if password is provided, confirmPassword must match
  // This refinement will be adjusted based on whether it's a new user or an edit
  return true; 
}, {});


// Dynamically adjust schema based on editing state
const getUserSchema = (isEditing: boolean, isEditingAdmin: boolean) => {
  return z.object({
    username: z.string().min(3, { message: 'El nombre de usuario debe tener al menos 3 caracteres.' }),
    password: z.string()
      .min(isEditing && !isEditingAdmin ? 0 : 6, { message: 'La contraseña debe tener al menos 6 caracteres.'}) // Min 0 if editing and not admin, to allow no change
      .optional(),
    confirmPassword: z.string().optional(),
  }).refine(data => {
    if (!isEditing) { // New user
      if (!data.password || data.password.length < 6) return false; // Password required for new user
      return data.password === data.confirmPassword;
    }
    if (data.password) { // Existing user, password change attempted
      if (isEditingAdmin) return true; // Admin password cannot be changed through this form
      if (data.password.length < 6) return false;
      return data.password === data.confirmPassword;
    }
    return true; // No password change attempted for existing user
  }, {
...
interface UserFormModalProps {
  ...
  allUsers: UserCredentials[]; // To check for username uniqueness
}
...
export default function UserFormModal({
    ...
    if (isOpen) {
      if (editingUser) {
        form.reset({
          ...
          password: '', // Password fields are for changing, not displaying existing
          confirmPassword: '',
        });
      } else {
        form.reset({
  ...
  const handleSubmit = (data: UserFormValues) => {
    ...
    const finalUserData: UserCredentials = {
      ...
      password_plaintext: (data.password && data.password.length > 0) ? data.password : (editingUser ? editingUser.password_plaintext : ''), // Fallback to existing if not changed
    };
    
    // Prevent changing admin password if current user is not admin or if they try anyway
    if (editingUser && editingUser.username === 'admin' && data.password && data.password.length > 0) {
        toast({
            title: "Operación no permitida",
            description: "La contraseña del administrador no puede ser cambiada.",
            variant: "destructive",
        });
        return;
    }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Ej: doctor.house" {...field} disabled={isEditingAdmin} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
